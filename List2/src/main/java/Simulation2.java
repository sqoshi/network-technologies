...
class Simulation2 {
    ...
    private double calculateAverageDelay(SimpleWeightedGraph<Integer, DefaultWeightedEdge> graph) {
        ...
        ArrayList<DefaultWeightedEdge> edges = new ArrayList<DefaultWeightedEdge>(graph.edgeSet());
    ...
    private SimpleWeightedGraph<Integer, DefaultWeightedEdge> MyGraph() {
        ...
        for (int i = 0; i < 10; i++) {
            graph.addVertex(i);
            if (i > 0)
                graph.setEdgeWeight(graph.addEdge(i - 1, i), 0.7);
        }
        graph.setEdgeWeight(graph.addEdge(0, 6), 0.9);
        graph.setEdgeWeight(graph.addEdge(1, 7), 0.9);
        graph.setEdgeWeight(graph.addEdge(2, 8), 0.9);
        graph.setEdgeWeight(graph.addEdge(3, 9), 0.9);
        graph.setEdgeWeight(graph.addEdge(5, 9), 0.9);
        graph.setEdgeWeight(graph.addEdge(0, 9), 0.9);
    ...
    private double networkCheck(SimpleWeightedGraph<Integer, DefaultWeightedEdge> graph) {
        ...
        double T_max = 2.5;//* calculateAverageDelay(graph);
        ...
        for (int i = 0; i < numberOfAttempts; i++) {
            ...
            ConnectivityInspector<Integer, DefaultWeightedEdge> inspector = new ConnectivityInspector<Integer, DefaultWeightedEdge>(graphToCheck);
